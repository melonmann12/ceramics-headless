...
async function test() {
  ...
  // 1. Get product to find a valid variant ID
  const productQuery = `
    query {
      productByHandle(handle: "the-matcha-bowl") {
        variants(first: 1) {
          edges {
            node {
              id
  ...
  const variantId = json.data.productByHandle.variants.edges[0].node.id;
  console.log('Variant ID:', variantId);
  
  // 2. Add to Cart (Create Cart)
